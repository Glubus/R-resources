...
pub fn preprocess_xml(xml: &str, current_profile: &str) -> String {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(e)) => {
                handle_start_event(&e, current_profile, &mut skip_depth, &mut out);
            }
            Ok(Event::Empty(e)) => {
                handle_empty_event(&e, current_profile, skip_depth, &mut out);
            }
            Ok(Event::Text(e)) => handle_text_event(&e, skip_depth, &mut out),
            Ok(Event::End(e)) => handle_end_event(&e, &mut skip_depth, &mut out),
            Ok(Event::Eof) => break,
            Ok(
                Event::Comment(_)
                | Event::Decl(_)
...
#[inline]
fn extract_profile_attr_start(e: &quick_xml::events::BytesStart) -> Option<String> {
...
#[inline]
fn extract_profile_attr_empty(e: &quick_xml::events::BytesStart) -> Option<String> {
    for attr in e.attributes().flatten() {
        if attr.key.as_ref() == b"profile" {
            return Some(String::from_utf8_lossy(&attr.value).to_string());
        }
...
#[inline]
fn handle_text_event(e: &quick_xml::events::BytesText, skip_depth: usize, out: &mut String) {
    if skip_depth == 0 {
...
#[inline]
fn handle_end_event(e: &quick_xml::events::BytesEnd, skip_depth: &mut usize, out: &mut String) {
    if *skip_depth > 0 {
        *skip_depth -= 1;
    } else {
        write_end_tag(out, e);
