...
pub fn generate_color_module(colors: &[(String, ResourceValue)]) -> String {
    ...
    struct Node<'a> {
        ...
        items: Vec<(&'a str, &'a ResourceValue)>,
    }
    ...
    fn emit_tnode(code: &mut String, node: &TNode, indent: usize) {
        ...
        for (k, child) in &node.children {
            let _ = writeln!(code, "{}pub mod {} {{", pad, k);
            emit_tnode(code, child, indent + 4);
            let _ = writeln!(code, "{}}}", pad);
        }
        for (leaf, hex) in &node.items {
            if let Some((a, r, g, b)) = parse_hex_argb(hex) {
                let _ = writeln!(
                    code,
                    "{}pub const {}: crate::Color = crate::Color::new({r}, {g}, {b}, {a});",
                    pad,
