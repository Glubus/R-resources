...
fn main() {
    ...
    println!("  full_api_url: {} (references @string/base_url)", string::FULL_API_URL);
    println!("  app_title: {} (references @string/app_name)", string::APP_TITLE);

    // Colors with references
    println!("\nColor references:");
    println!("  accent: {}", color::ACCENT);
    println!("  button_color: {} (references @color/accent)", color::BUTTON_COLOR);

    // Verify references work correctly
    println!("\n=== Verification ===");
    println!("full_api_url == base_url: {}", string::FULL_API_URL == string::BASE_URL);
    println!("app_title == app_name: {}", string::APP_TITLE == string::APP_NAME);
    println!("button_color == accent: {}", color::BUTTON_COLOR == color::ACCENT);
