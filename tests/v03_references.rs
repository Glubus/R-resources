...
fn test_reference_chains() {
    ...
    assert_eq!(r::APP_TITLE, "My Awesome App");
    assert_eq!(r::FULL_API_URL, "https://api.example.com");
}

#[test]
fn test_cross_type_resource_access() {
    // Verify that all resource types coexist properly by touching them
    std::hint::black_box(r::BASE_URL);
    std::hint::black_box(r::ACCENT);
    std::hint::black_box(r::MAX_RETRIES);
